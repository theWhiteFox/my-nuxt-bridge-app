...
async function handleError(error, req, res) {
  ...
  const errorObject = {
    ...
    description: isDev && !is404 ? `<pre>${stack.map(i => `<span class="stack${i.internal ? " internal" : ""}">${i.text}</span>`).join("\n")}</pre>` : ""
  };
  ...
  const url = (0, _ufo.withQuery)("/_nitro/__error", errorObject);
  const html = await (0, _.$fetch)(url).catch(() => errorObject.statusMessage);
